define(['lib!Widget', './file', './video', './subText', './dialog', 'lib!funcUtils'],
  (Widget, File, Video, SubText, Dialog, funcUtils) => {
    ...
    return class Controller extends Widget {
      ...
      on_time_update() {
        var time = video.time();
        (window.WebPortal || localStorage).setItem('time', time);
      ...
      init() {
        ...
        this.$window = $(window);
        this.index = ((window.WebPortal || localStorage).getItem('index') || 0) * 1;
        subText.subTitle.syncVal = ((window.WebPortal || localStorage).getItem('syncVal') || 0) * 1;
        this.timeA = null;
        this.timeB = null;
      }
      play_init() {
        this.index = 0;
        (window.WebPortal || localStorage).setItem('index', this.index);
        (window.WebPortal || localStorage).setItem('time', 0);
        this.play();
      }
      play(keep) {
        if (file.has_index(this.index, this.get_file_extention())) {
          video.src(file.url(this.index, this.get_file_extention()));
          video.time(((window.WebPortal || localStorage).getItem('time') || 0) * 1);
          this.load_sub();
      ...
      check_key_down(key) {
        switch (key) {
          ...
          case 'b': if (file.has_index(this.index - 1, this.get_file_extention())) { video.src(file.url(--this.index, this.get_file_extention())); (window.WebPortal || localStorage).setItem('index', this.index); this.load_sub(); } this.alert_play_info(); break;
          case 'n': if (file.has_index(this.index + 1, this.get_file_extention())) { video.src(file.url(++this.index, this.get_file_extention())); (window.WebPortal || localStorage).setItem('index', this.index); this.load_sub(); } this.alert_play_info(); break;
          ...
          case '+': subText.sync(1); (window.WebPortal || localStorage).setItem('syncVal', subText.sync()); this.alert_sync(); break;
          case '-': subText.sync(-1); (window.WebPortal || localStorage).setItem('syncVal', subText.sync()); this.alert_sync(); break;
